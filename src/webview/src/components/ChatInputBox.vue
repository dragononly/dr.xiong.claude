...
function handleInput(event: Event) {
  ...
  // 更新 dropdown 位置（锚定在触发词开头）- 使用 requestAnimationFrame 优化性能
...
// 缓存上次的高度，避免不必要的 DOM 操作
let lastTextareaHeight = 20
...
function autoResizeTextarea() {
  ...
  if (newHeight !== lastTextareaHeight) {
    ...
    divElement.style.overflowY = scrollHeight > maxHeight ? 'auto' : 'hidden'
  } else {
    // 恢复原高度
    divElement.style.height = `${newHeight}px`
  }
