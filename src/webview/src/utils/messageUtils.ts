...
// 🚀 性能优化：使用 Map 缓存 tool_use 位置，避免 O(n²) 遍历
const toolUseCache = new Map<string, ContentBlockWrapper>();

/**
 * 清除 tool_use 缓存（在会话切换时调用）
 */
export function clearToolUseCache(): void {
  toolUseCache.clear();
}
 ...
 * 缓存 tool_use block
 */
function cacheToolUse(wrapper: ContentBlockWrapper): void {
  if (wrapper.content.type === 'tool_use') {
    const id = (wrapper.content as ToolUseContentBlock).id;
    if (id) {
      toolUseCache.set(id, wrapper);
    }
  }
}

/**
 * 反向查找 tool_use block（优化版：优先使用缓存）
 *
 * 对应原始代码：
 * function rZe(n, e) {
...
export function findToolUseBlock(
    ...
    // 🚀 优先从缓存查找
    const cached = toolUseCache.get(toolUseId);
    if (cached) {
      return cached;
    }
            ...
            if (Array.isArray(content)) {
                for (const wrapper of content) {
                    ...
                    if (
                        ...
                        // 找到后加入缓存
                        toolUseCache.set(toolUseId, wrapper);
                        return wrapper;
                    }
                }
            }
...
export function processAndAttachMessage(messages: Message[], rawEvent: any): void {
    ...
    if (message) {
        ...
        // 🚀 性能优化：新消息中的 tool_use 加入缓存
        if (message.type === 'assistant' && Array.isArray(message.message.content)) {
            for (const wrapper of message.message.content) {
                cacheToolUse(wrapper);
            }
        }
    }
}
